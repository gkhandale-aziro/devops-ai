"""
providers.py — AI provider layer using LiteLLM with native tool calling.
Same approach as kubectl-ai's gollm/ollama.go — proper tool protocol, no regex.
...
Examples:
  ...
  TOOL_MODEL=ollama/qwen2.5:7b  ANSWER_MODEL=ollama/mixtral:8x7b python3 main.py

  # Cloud models
  AI_MODEL=claude-haiku-4-5-20251001 python3 main.py
  TOOL_MODEL=ollama/llama3.1:8b ANSWER_MODEL=claude-haiku-4-5-20251001 python3 main.py
"""
import os
import json
import time
...
# TOOL_MODEL  — used for the agentic loop (deciding what commands to run)
# ANSWER_MODEL — used for the final answer (analysis, summary, recommendations)
# If only AI_MODEL is set, both phases use the same model
...
TOOLS = [
    {
        ...
        "function": {
            "name": "run_command",
            "description": (
                "Run a shell command on this Linux machine and return the real output. "
